      ...
      {members.map((member) => {
        ...
        return (
          <Link
            ...
            className="flex items-start gap-2.5 p-2 rounded-lg hover:bg-bg-tertiary/50 transition-colors"
          >
            <div
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                ...
                {member.is_red_team && (
                  <Badge variant="critical" className="text-[9px] px-1 py-0">RT</Badge>
                )}
              </div>
              ...
              {member.expertise_tags.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-1">
                  {member.expertise_tags.slice(0, 3).map((tag) => (
                    <span key={tag} className="text-[9px] px-1.5 py-0.5 rounded bg-bg-tertiary text-text-muted">
                      {tag}
                    </span>
                  ))}
                  {member.expertise_tags.length > 3 && (
                    <span className="text-[9px] text-text-muted">+{member.expertise_tags.length - 3}</span>
                  )}
                </div>
              )}
